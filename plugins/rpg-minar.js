let handler = async (m, { conn }) => {
  ...
  // Obtener el factor de aumento basado en el nivel del pico (con validación)
  ...
  minerales.forEach(recursoAleatorio => {
    ...
    // Verificar que cantidadAleatoria es un número válido antes de aplicar el factor de aumento
    ...
    // Verificar que gananciaAdicionalPico es un número válido
    ...
    // Verificar que cantidadAleatoria sigue siendo un número válido
    if (isNaN(cantidadAleatoria)) {
...
function obtenerFactorAumentoPico(nivelPico) {
  // Si el nivel del pico no es un número válido, asignamos un valor predeterminado
  if (isNaN(nivelPico) || nivelPico <= 0) {
    nivelPico = 1;  // Asignamos el valor 1 si el nivel del pico no es válido
  }
